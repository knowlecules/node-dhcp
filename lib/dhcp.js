...
Server.prototype = {
  ...
  _selectAddress: function(clientMAC) {
    ...
    const reReservedLastOctals = /\.(255|254|0|1)\s*$/;
    ...
    var firstIpNum = Tools.parseIp(_tmp[0]);
    var lastIpNum = Tools.parseIp(_tmp[1]);
    var highestIpNum = firstIpNum;


    // Add all known addresses and save the oldest lease
    this._ips[this.config('server')] = true; // Exclude our own server IP from pool
    var oldestMac = null;
    ...
    if (lastIpNum - firstIpNum === leaseCount) {
      var ip = this._state[oldestMac].address;

      _removeLease(oldestMac)
      return ip;
    }
    ...
    if (randIP) {
      ...
      while (1) {
        ...
        var ip = Tools.formatIp(firstIpNum + Math.random() * (lastIpNum - firstIpNum) | 0);

        if (!reReservedLastOctals.test(ip) && !_ips[ip]) {
          return ip;
        }
      }
    }
    ...
    // Choose first free IP in subnet after highest previosuly allocated
    for (var i = highestIpNum; i <= lastIpNum; i++) {
    ...
    // Choose first free IP in subnet before highest previosuly allocated
    for (var i = firstIpNum; i <= highestIpNum; i++) {
  ...
  _removeLease : function(mac) {
    // TODO: Notify deleted client
    delete this._ips[this._state[mac].ipAddress];
    delete this._state[mac];
  },

  allocateIP : function ({ipAddress, mac, leaseState = 'BOUND', leasePeriod = 0, bindTime = 0}) {
    var lease = this._state[mac] = this._state[mac] || new Lease;
    lease.address = ipAddress;
    lease.leasePeriod = leasePeriod || this.config('leaseTime');
    lease.server = this.config('server');
    lease.state = leaseState;
    highestIpNum = Tools.parseIp(lease.address);
    this._ips[lease.address] = lease;
  },

  handleDiscover: function(req) {
    //console.log('Handle Discover', req);
    allocateIP({
      ipAddress  : this._selectAddress(req.chaddr),
      mac : req.chaddr,
      leaseState : 'OFFERED',
    })
    this.sendOffer(req);
  ...
  handleRequest: function(req) {
    ...
    allocateIP({
      ipAddress  : this._selectAddress(req.chaddr),
      mac : req.chaddr,
      leasePeriod = this.config('leaseTime'),
      leaseState : 'BOUND',
      bindTime : new Date,
    })
