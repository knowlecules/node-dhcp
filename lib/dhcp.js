...
Server.prototype = {
  ...
  config: function(key) {
    ...
    if (key !== 'range' && key !== 'static' && key !== 'randomIP' && key !== 'provider' && Options.opts[optId].enum) {
  ...
  _generateAddress: function(clientMAC){
  ...
  _selectAddress: function(clientMAC) {
     ...
     */
     ...
     //User provider if defined
     try {
       if(this.config('provider').get(clientMAC)){
         return this.config('provider').get(clientMAC);
       } else {
         ip = this._generateAddress(clientMAC);
         this.config('provider').set(clientMAC, ip);
         return ip;
       }
     } catch(err) {
       //Store in Memory
       ...
       if (this._state[clientMAC] && this._state[clientMAC].address) {
         ...
         return this._generateAddress(clientMAC);
       }
     }
...
module.exports = {
  ...
  FileProvider: function(path){
    return new require('../providers/file.js')(path);
  },
